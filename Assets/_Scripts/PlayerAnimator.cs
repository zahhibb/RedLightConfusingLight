...
public class PlayerAnimator : MonoBehaviour
{
    ...
    [SerializeField]
    private float _maxParticleFallSpeed = -40;
    ...
    void Update()
    {
        ...
        if (_player.Input.X != 0)
            transform.localScale =
                new Vector3(_player.Input.X > 0 ? 1 : -1, 1, 1);
        ...
        _anim.transform.rotation =
            Quaternion
                .RotateTowards(_anim.transform.rotation,
                Quaternion.Euler(targetRotVector),
                _tiltSpeed * Time.deltaTime);

        // Speed up idle while running
        _anim
            .SetFloat(IdleSpeedKey,
            Mathf.Lerp(1, _maxIdleSpeed, Mathf.Abs(_player.Input.X)));
        ...
        if (_player.LandingThisFrame)
        {
            ...
            if (_source != null)
                _source
                    .PlayOneShot(_footsteps[Random
                        .Range(0, _footsteps.Length)]);
        }
        ...
        // Detect ground color
        var groundHit =
            Physics2D.Raycast(transform.position, Vector3.down, 2, _groundMask);
        if (
            groundHit &&
            groundHit.transform.TryGetComponent(out SpriteRenderer r)
        )
        {
            _currentGradient =
    ...
    private static readonly int
        IdleSpeedKey = Animator.StringToHash("IdleSpeed");
